...
STATIC_education_DIR = BASE_DIR / 'static' / 'education'
STATIC_education_COVERS_DIR = BASE_DIR / 'static' / 'covers' / 'education'

@bp.route('/all_education_genre', methods=['GET'])
def get_all_education_genre():
    """
    获取所有教育的分类
    返回：
    - 所有教育的分类列表
    """
    try:
        # 获取所有教育的分类（直接透出的话是没有分类信息的，因为分类信息没有落到Coneten中的）
        # 有两种做法能够满足这个目录或者content所属的分类到底是热血教育还是冷门教育：
        ...
        # 当前我们使用resource中的education_mapping_x.json 来获取所有教育的分类
        '''
        from app.models.content import Content
        from sqlalchemy import desc
        
        educations = Content.query.filter_by(type='education')\
            .order_by(desc(Content.updated_at))\
            .all()
        
        # 将查询结果转换为字典列表
        all_education_genres = [education.to_dict() for education in educations]
        
        return api_response(message='获取所有教育的分类成功', data=all_education_genres)
        '''
        ...
        total_educations_count = 0
        
        # 扫描resource目录下的education_mapping文件
        for file_path in RESOURCE_DIR.glob('education_mapping_*.json'):
            try:
                # 读取JSON文件
                ...
                # 处理每个教育分类
                for category, educations in data.items():
                    ...
                    for education in educations:
                        if isinstance(education, dict) and 'name' in education:
                            # 检查是否有封面URL，如果没有或者是默认封面，尝试从static/covers/education目录获取
                            education_name = education['name']
                            total_educations_count += 1
                            
                            # 检查是否已有非默认封面
                            has_custom_cover = ('cover_url' in education and 
                                               education['cover_url'] and 
                                               not education['cover_url'].endswith('default_cover.jpg'))
                            
                            if not has_custom_cover:
                                # 检查是否存在对应的封面目录
                                education_cover_dir = STATIC_education_COVERS_DIR / education_name
                                if education_cover_dir.exists() and education_cover_dir.is_dir():
                                    # 获取目录下所有jpg和png文件
                                    cover_images = list(education_cover_dir.glob('*.jpg')) + list(education_cover_dir.glob('*.png'))
                                    if cover_images:
                                        # 随机选择一张图片作为封面
                                        cover_image = random.choice(cover_images)
                                        relative_path = f"/static/covers/education/{education_name}/{cover_image.name}"
                                        education['cover_url'] = relative_path
                                    else:
                                        # 目录存在但没有图片，使用默认封面
                                        education['cover_url'] = "/static/covers/education/default_cover.jpg"
                                        default_cover_count += 1
                                else:
                                    # 目录不存在，使用默认封面
                                    education['cover_url'] = "/static/covers/education/default_cover.jpg"
                                    default_cover_count += 1
                            elif education['cover_url'].endswith('default_cover.jpg'):
                                default_cover_count += 1
                                
                            # 不需要在每个教育对象中添加category字段，因为现在已经按分类组织了
                            genres_dict[category].append(education)
        
            except Exception as e:
                print(f'Error processing {file_path}: {e}')
        
        # 检查默认封面的比例，如果超过50%，输出警告日志
        if total_educations_count > 0:
            default_cover_ratio = default_cover_count / total_educations_count
            if default_cover_ratio > 0.5:  # 如果超过50%使用默认封面
                logging.warning(f"警告：有{default_cover_count}/{total_educations_count}({default_cover_ratio:.2%})的教育使用默认封面，请管理员更新封面资源！")
        
        # 将字典转换为前端需要的格式：[{category: '分类名', educations: [教育列表]}]
        formatted_genres = []
        for category, educations in genres_dict.items():
            formatted_genres.append({
                'category': category,
                'educations': educations
            })
        
        return api_response(message='获取所有教育的分类成功', data=formatted_genres)
        
    except Exception as e:
        return api_response(message=f'获取教育分类失败: {str(e)}', code=500)
...
def get_content_detail():
    ...
    try:
        ...
        # 这里的content_id可能是content中的name（特别是教育的Category导航的逻辑中，将教育的名称作为content name，并且传递到这里面来作为导航项的，所以这里必须可能的教育名称作一个特殊的处理）
        ...
        if not content:
            # 尝试使用name查询
            contents = ContentService.get_contents_by_name_and_type(content_id, 'EDUCATION')
