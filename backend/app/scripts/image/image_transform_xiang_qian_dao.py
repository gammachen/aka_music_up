...
def apply_perspective_transform(img, strength=0.3):
    ...
    y_offset_top = -int(height * strength * 0.8)  # 顶部Y轴偏移量（负值使其向后倾斜）
    
    # 同时添加一些水平方向的收缩，增强3D效果
    x_offset = int(width * strength * 0.3)  # 水平方向的收缩量
    
    # 调整四个角点的位置，创造更真实的向后倒的3D效果
    # 顶部向后倾斜，底部略微向前，增强3D效果
    dst_points = np.float32([
        [x_offset, y_offset_top],                # 左上
        [width - 1 - x_offset, y_offset_top],    # 右上
        [int(width * 0.05), height - 1],         # 左下（略微向内）
        [width - 1 - int(width * 0.05), height - 1]  # 右下（略微向内）
    ])
    ...
    # 应用透视变换，确保使用白色背景
    # 增加输出图像的高度，以容纳向后倒的效果
    # 由于是向后倒，需要更大的高度来容纳上方延伸的部分
    output_height = int(height * 1.3)
    # 调整输出图像的起始位置，使图像主体保持在视野中
    output_matrix = perspective_matrix.copy()
    output_matrix[1, 2] += abs(y_offset_top)  # 向下移动图像，确保顶部不会被裁剪
