...
def apply_perspective_transform(img, strength=0.3):
    ...
    # 目标点：顶部收窄，底部变宽，创造透视效果
    # 对于已经斜切过的图像，我们需要调整透视效果
    offset_top = int(width * strength / 2)  # 顶部收窄的偏移量
    # 调整左上和右上点的水平位置，使透视效果更加自然
    dst_points = np.float32([
        [offset_top, 0],                # 左上
        [width - 1 - offset_top, 0],    # 右上
        [0, height - 1],                # 左下
        [width - 1, height - 1]         # 右下
    ])
    ...
    # 应用透视变换，确保使用白色背景
    result = cv2.warpPerspective(img, perspective_matrix, (width, height), borderValue=(255, 255, 255))
