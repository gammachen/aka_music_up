...
def apply_perspective_transform(img, strength=0.3):
    ...
    # 为了使透视点在右侧，左上角的x偏移较小，右上角的x偏移较大
    left_x_offset = int(x_offset * 0.3)  # 左侧偏移较小
    right_x_offset = int(x_offset * 1.7)  # 右侧偏移较大
    dst_points = np.float32([
        [left_x_offset, y_offset_top],           # 左上（Y值增大，向下移动，X值偏移较小）
        [width - 1 - right_x_offset, y_offset_top],  # 右上（Y值增大，向下移动，X值偏移较大）
        [0, height - 1],                         # 左下（保持原位置）
...
def skew_and_rotate_image(input_path, output_path, skew_factor=0.3, rotation_angle=-15, bg_color=(255, 255, 255), perspective_strength=0.0):
    """
    对图片进行斜切、透视和旋转处理
    ...
    try:
        ...
        # 应用旋转变换
        rotated_img = cv2.warpAffine(skewed_img, rotation_matrix, (new_width, new_height), borderValue=bg_color)
        
        # 2. 再应用透视变换（如果需要）
        result_img = rotated_img
        if perspective_strength > 0:
            result_img = apply_perspective_transform(rotated_img, perspective_strength)
...
def main():
    ...
    parser.add_argument('-r', '--rotate', type=float, default=-15, help='旋转角度 (默认: -15度)')
