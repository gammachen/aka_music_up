...
class ZhipuAI:
    ...
    def __init__(self, api_key: str):
        ...
        self.api_url = "https://open.bigmodel.cn/api/paas/v4/chat/completions"  # 智谱AI的API地址
    ...
    def _make_request_with_retry(self, payload: Dict) -> Tuple[Dict, bool]:
        """带重试机制的请求
    ...
    def analyze_poem(self, poem: Dict) -> Dict:
        ...
        # 使用重试机制发送请求
        result, success = self._make_request_with_retry(payload)
...
def enrich_poems_with_ai(poems_file_path: str, api_key: str, output_file_path: Optional[str] = None, checkpoint_file: Optional[str] = None) -> str:
    """使用智谱AI丰富诗歌信息
    ...
    try:
        for i in range(start_index, len(poems)):
            ...
            try:
                # 获取AI分析结果
                background_info = zhipu_ai.analyze_poem(poem)
...
def generate_timeline_html(enriched_poems: List[Dict], output_html_path: str, poet_name: str, poet_intro: str) -> None:
    ...
    """
    # 按创作年份排序
    sorted_poems = sorted(enriched_poems, key=lambda x: x.get('creation_year', 9999))
...
def analyze_poet_poems(poet_name: str, input_json_path: str, output_json_path: str, output_html_path: str, poet_intro: str, api_key: str):
    """完整的诗人诗歌分析流程
    ...
    enriched_file_path = enrich_poems_with_ai(output_json_path, api_key)
...
def main():
    ...
    # 执行完整分析流程
    analyze_poet_poems(poet_name, input_json_path, output_json_path, output_html_path, poet_intro, api_key)
