...
def annotate_image(image_path, output_image_path, result):
    ...
    # 在右侧显示提取的文本
    text_content = "\n".join([f"{detection['text']} ({detection['confidence']:.2f})" for detection in image_metadata['detections']])
...
def generate_md_document(metadata, output_dir):
    ...
    with open(md_path, 'w', encoding='utf-8') as f:
        ...
        for item in metadata:
            ...
            f.write(f"![{item['filename']}](annotated_{item['filename']})\n\n")  # 插入图片
            f.write("### 识别结果\n")
            f.write("| 文本 | 坐标 | 置信度 |\n")
            f.write("|------|------|--------|\n")
...
if __name__ == "__main__":
    ...
    # p2t predict -l en,ch_sim --mfd-config '{"model_name": "mfd-pro", "model_backend": "onnx"}' --formula-ocr-config '{"model_name":"mfr-pro","model_backend":"onnx"}' --text-ocr-config '{"rec_model_name": "doc-densenet_lite_666-gru_large"}' --rec-kwargs '{"page_numbers": [0, 1]}' --resized-shape 768 --file-type pdf -i pix2text/pdf/shenduxuexi_deeplearning.pdf -o pix2text/pdf/output-md --save-debug-res output-debug
    
    # p2t predict -l en,ch_sim --rec-kwargs '{"page_numbers": [5, 10]}' --resized-shape 768 --file-type pdf -i pix2text/pdf/shenduxuexi_deeplearning.pdf -o pix2text/pdf/output-md --save-debug-res output-debug
    
    #  python easyocr_intro_v2.py --input_dir licence_plates --output_dir licence_plates_recognition